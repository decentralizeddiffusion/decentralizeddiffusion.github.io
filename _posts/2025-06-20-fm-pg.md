...
#       name: UC Berkeley
#   - name: Songwei Ge
#     url: "https://songweige.github.io"
#   - name: Brent Yi
#     url: "https://brentyi.github.io/"
#   - name: Haven Feng
#     url: "https://havenfeng.github.io"
#   - name: Ethan Weber
#     url: "https://ethanweber.me"
#   - name: Chung Min Kim
#     url: "https://chungmin99.github.io"
#   - name: Hongsuk Choi
#     url: "https://hongsukchoi.github.io/"
...
Flow models have become the go-to approach to model distributions in continuous space. They soak up data with a simple, scalable denoising objective and now represent the state-of-the art in generating image, video, audio and, more recently, robot action. However, they are still unpopular for learning from rewards through reinforcement learning.

To perform RL in continuous spaces, practicioners typically train far simpler Gaussian policies, which represent a single, ellipsoidal mode of the action distribution. Alternatively, flow-based policies can capture complex, multimodal action distributions. We show that it's possible to train policies using flow matching, the framework behind modern diffusion and flow models, to benefit from its expressivity.
...
We introduce <b>Flow Policy Optimization</b> (FPO), a new algorithm to train RL policies with flow matching. It can train expressive policies from scratch in simulation or finetune existing flow/diffusion models, using their pretraining knowledge as a prior for exploration. We find its particularly useful to learn underconditioned policies, like humanoid locomotion with simple joystick commands.
...
<div class="l-body" style="text-align: center; margin-top: -12%; margin-bottom: -12%;">
  <img src="{{ '/assets/img/fpo/flow_matching.svg' | relative_url }}" alt="DDM Overview" style="margin-left: -10%; width: 120%; height: auto; clip-path: inset(0px 0 0px 0);">
</div>
...
Flow matching perturbs a clean data sample with Gaussian noise then tasks the model with reconstructing the sample by predicting the velocity, which is the derivative of $x_t$ *w.r.t.* time. In expectation over a fixed dataset, this optimization recovers the marginal flow for any $x_t$.
...
This procedure climbs the policy gradient---the gradient of expected cumulative reward. Your model collects "experience" from its distribution, sees which samples are most advantageous, and adjusts to perform similar actions more often.
...
From the rewards, we calculate advantages. These can be viewed as the reward normalized *w.r.t.* the expected reward from the rest of the rollout under the current policy. This expected reward is what you learn with a critic in PPO or compute as the average of a group's reward in GRPO.
...
To reiterate, the goal of on-policy RL is simple: increase the likelihood of high-reward actions. Flow matching naturally increases likelihoods by redirecting probability flow toward training samples. This makes our objective clear---<b>redirect flow toward high reward actions</b>.

In the limit of perfect optimization, flow matching assigns probabilities according to the frequency of samples in your training set. In our case, that "training set" is dynamically generated from the model.

Advantages make the connection between synthetic data generation and on-policy RL explicit. In RL, we calculate the advantage of each data point, a quantity that indicates how much it was than expected. These advantages are typically centered around zero to reduce variance: positive for better-than-expected actions, negative for worse. Advantages then become a *loss weighting* in the policy gradient. As a simple example, if an action is very advantageous, the model encounters a scaled-up loss on it and learn to boost it aggressively.

<div class="l-body" style="text-align: center;">
  <img src="{{ '/assets/img/fpo/pg.png' | relative_url }}" alt="DDM Overview" style="width: 80%; height: auto;">
</div>
<div class="caption">
    Placeholder, will show that policy gradient incorporates the advantage as a loss weighting.
</div>

Zero-mean advantages are fine for RL in discrete spaces since a negative advantage just pushes down the model's predicted likelihood of an action. Flow matching, however, learns probability flows to sample from a training data distribution. Probabilities must be nonnegative, so negative loss weights break the interpretation.

There's a simple solution: make the advantages nonnegative. Shifting advantages by a constant doesn't change the policy gradient. This is actually the same mathematical property that lets us use advantages instead of raw rewards in the first place! Here's how we can understand non-negative advantages in the flow matching framework:
...
<div class="l-page">
 ...
 So why not redirect that flow toward high-reward actions instead?



This is problematic for exploration. Imagine a situation where there are two high-reward modes, the policy can try to span them by predicting a Gaussian with its mean between them

We adapt flow matching—the framework behind modern diffusion and flow models—to work with policy gradient methods.

Reinforcement learning typically 
